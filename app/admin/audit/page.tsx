...
export default async function AdminAuditPage() {
  ...
  const auth = await requireRole(session, ['ADMIN']);
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto bg-slate-900 min-h-[calc(100vh-4rem)]">
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div>
          <h1 className="text-2xl font-black text-slate-100">Global System Audit Logs</h1>
          <p className="text-xs text-slate-400">
            Real-time audit trailing for compliance, security access control, and records governance
          </p>
        </div>
        <Badge variant="purple">{logs.length} Audit Entries</Badge>
      </div>

      <div className="glass-panel rounded-2xl border border-slate-800 overflow-hidden bg-slate-950/50">
